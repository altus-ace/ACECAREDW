...
FROM		(SELECT DISTINCT 
		 ...
		 PR.AccountType AS PCP_AccountType,
           ...
           MbrPcp.NPI,
           pr.PrimaryAddressPhoneNumber AS [PCP_PHONE],
           '' AS [PCP_FAX],
    ...
    FROM [adw].[MbrMember] m    
	   ...
	   LEFT JOIN( SELECT pr.npi, pr.LastName, PR.FirstName, pr.AttribTIN TIN, pr.ProviderSpecialty
				, pr.ProviderSubSpecialty, pr.AttribTINName AS GroupName, pa.PrimaryAddress, pa.PrimaryCity
				, pa.PrimaryState, pa.PrimaryZipcode, pa.PrimaryPOD, pa.PrimaryQuadrant, pa.PrimaryAddressPhoneNumber				
				, pr.ClientKey, pr.AccountType, '' NetworkContact
				FROM adw.tvf_AllClient_ProviderRoster(0, @effectiveDate, 1) pr
				    JOIN adw.FctProviderPracticePrimaryAddress pa
					   ON pr.AttribTIN = pa.TIN) AS PR 
