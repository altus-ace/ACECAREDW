...
GO
CREATE NONCLUSTERED INDEX [ndx_fctProvRoster_ProvTypeRowEffExNpiTinClientKey]
    ON [adw].[fctProviderRoster]([ProviderType] ASC, [RowExpirationDate] ASC, [RowEffectiveDate] ASC, [NPI] ASC, [TIN] ASC, [ClientKey] ASC)
    INCLUDE([fctProviderRosterSkey], [LastName], [FirstName], [GroupName], [PrimaryAddress], [PrimaryCity], [PrimaryState], [PrimaryZipcode], [PrimaryPOD], [PrimaryQuadrant], [PrimaryAddressPhoneNum], [AccountType]);
...
GO
CREATE NONCLUSTERED INDEX [_dta_index_fctProviderRoster_6_1062398954__K12_K39_K10_K23_K24_K11_K43_K19_K1]
    ON [adw].[fctProviderRoster]([ClientKey] ASC, [HealthPlan] ASC, [RowEffectiveDate] ASC, [EffectiveDate] ASC, [ExpirationDate] ASC, [RowExpirationDate] ASC, [ProviderType] ASC, [TIN] ASC, [fctProviderRosterSkey] ASC);
...
GO
CREATE NONCLUSTERED INDEX [ndx_FctProvRost_HpREffRExpEffExp]
    ON [adw].[fctProviderRoster]([HealthPlan] ASC, [RowEffectiveDate] ASC, [RowExpirationDate] ASC, [EffectiveDate] ASC, [ExpirationDate] ASC)
    INCLUDE([ClientKey], [TIN], [GroupName]);


GO
CREATE NONCLUSTERED INDEX [ndx_FctProvRost_ClientKeyRwEffRwExp]
    ON [adw].[fctProviderRoster]([ClientKey] ASC, [RowEffectiveDate] ASC, [RowExpirationDate] ASC)
    INCLUDE([TIN], [EffectiveDate], [ExpirationDate], [AccountType]);
