...
DECLARE @ClientKey INT = 12
					...
					--  DECLARE @LoadDate DATE = '2021-06-01' DECLARE @ClientKey INT = 12 
					...
					EXECUTE  [adi].[GetMbrNpiAndTin_CignaMA] @LoadDate,0,@ClientKey 
	...
	-----1st Membership Validation
		...
		FROM	(
			 ...
			 LEFT	JOIN	adw.tvf_AllClient_ProviderRoster(12,'2021-06-01',1) pr
			 ON				m.NPID = pr.NPI
			 LEFT JOIN		(SELECT * 
								FROM lst.lstPlanMapping pl
								WHERE ClientKey = 12
								AND  TargetSystem = 'ACDW')	pln
			ON				m.plan_name = pln.SourceValue
			WHERE			m.DataDate = '2021-06-01'
			--ORDER BY		m.memberid
			)src
			WHERE RwCnt = 1
			AND	NPI IS NOT NULL
