...
(
    ...
    FROM dbo.UHC_MembersByPCP AS mpcp 
	   ...
	   JOIN (SELECT pr.CalcClientKey, pr.TIN , pr.AccountType as AccountType
			 ...
			 WHERE pr.CalcClientKey = 1--.HealthPlan = 'UHC'
				--AND pr.ProviderType in ('PCP')
				--AND (getdate()) BETWEEN pr.EffectiveDate and pr.ExpirationDate -- anchor to the Last Update Date from the member table
