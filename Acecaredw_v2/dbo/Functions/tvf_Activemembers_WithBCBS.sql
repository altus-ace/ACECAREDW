CREATE FUNCTION [dbo].[tvf_Activemembers_WithBCBS](@EffectiveDate DATE) 
RETURNS TABLE
...
(   --declare @EffectiveDate date = getdate();
    ...
    FROM [adw].[MbrMember] m    
	   ...
	   LEFT JOIN(SELECT PR.NPI, PR.LastName, PR.FirstName, PR.TIN, PR.PrimarySpeciality, 
		...
		Left Join (Select * from [adw].[vw_ActiveMembers_ClientRiskScore]) rs
 ...
 --declare @EffectiveDate date = '12/15/2020'
   SELECT		 
    ...
    UNION
     SELECT		 
					c.MbrMemberKey AS [mbrMemberKey], 
					c.[MbrDemographicKey] AS mbrDemographicKey, 
					c.[MbrPlanKey] AS mbrPlanKey,
					c.[MbrPCPKey] AS mbrPcpKey,
					c.[MbrCsPlanKey] AS mbrCsPlanKey,
					c.[MbrPhoneKey_Home] AS MbrPhoneKeyType1,
					c.[MbrPhoneKey_Mobile] AS MbrPhoneKeyType4, 
					c.[MbrPhoneKey_Work] AS MbrPHoneKeyType3,
					c.[MbrAddressKey_Home] AS MbrAddressKey1, 
					c.[MbrAddressKey_Work] AS MbrAddressKey2, 
					'SHCN_MSSP' AS CLIENT,
					c.ClientKey		  AS ClientKey,
					c.ClientMemberKey  AS MEMBER_ID,
					Ace_ID	  AS Ace_ID,
					...
					c.MemberHomeZip AS [MEMBER_HOME_ZIP_C],
					c.MemberHomePhone AS [MEMBER_HOME_PHONE],
					'' AS [MEMBER_MAIL_ADDRESS],
					'' AS [MEMBER_MAIL_ADDRESS2],
					'' AS [MEMBER_MAIL_CITY],
					'' AS [MEMBER_MAIL_STATE],
					'' AS [MEMBER_MAIL_ZIP_C],
					c.MemberCellPhone AS [MEMBER_MAIL_PHONE],
					c.CountyName AS [MEMBER_COUNTY],
					'' AS [MEMBER_BUS_PHONE],
					' ' AS [MEMBER_ORG_EFF_DATE],
					' ' AS [MEMBER_CONT_EFF_DATE],
					c.MemberCurrentEffectiveDate AS [MEMBER_CUR_EFF_DATE],
					c.MemberCurrentExpirationDate AS [MEMBER_CUR_TERM_DATE],
					'' AS [PCP_CLIENT_ID],
					'' AS PCP_AccountType,
					c.ProviderFirstName AS [PCP_FIRST_NAME],
					c.ProviderLastName AS [PCP_LAST_NAME],
					...
					'' AS [MEMBER_MONTH_COUNT],
					'' AS [MEMBER_POD_C],
					c.ProviderChapter AS [MEMBER_POD_DESC],
					c.ProviderChapter AS [PCP_POD_C],
					c.ProviderChapter AS [PCP_POD_DESC],
					c.ProviderFirstName + ' ' + c.ProviderLastName AS [PCP_NAME],
					'' AS [CLIENT_UNIQUE_SYSTEM_ID],
					c.ClientMemberKey AS [MEDICARE_ID],
					c.RespParty_LastName AS [RESP_LAST_NAME],
					c.RespParty_FirstName AS [RESP_FIRST_NAME],
					c.RespParty_Address1 AS [RESP_ADDRESS],
					c.RespParty_Address2 AS [RESP_ADDRESS2],
					c.RespParty_City AS [RESP_CITY],
					c.RespParty_State AS [RESP_STATE],
					c.RespParty_Zip AS [RESP_ZIP],
					c.RespParty_Phone AS [RESP_PHONE],
					'' AS CurMonthsOld,
					c.CurrentAge AS CurYearsOld,  
		 			c.PlanID AS AhsPlanCode,
		 			c.PlanName AS AhsPlanName				
		 
    FROM ACDW_CLMS_SHCN_BCBS.adw.VW_FctMembership c		
	   LEFT JOIN ACDW_CLMS_SHCN_MSSP.adw.fctProviderRoster PR  -- Changed to Left Join to not remove TINS 
		  ON c.PcpPracticeTIN = pr.TIN
		  AND c.NPI = pr.NPI
		  AND c.RowEffectiveDate between pr.RowEffectiveDate and pr.RowExpirationDate		 
    WHERE (SELECT CASE WHEN (@effectiveDate > 
				( SELECT Max(c.RwExpirationDate)  FROM ACDW_CLMS_SHCN_MSSP.adw.FctMembership c)) 
				    THEN (SELECT Max(c.RwExpirationDate) FROM ACDW_CLMS_SHCN_MSSP.adw.FctMembership c)
				    ELSE @EffectiveDate END as aDate)	BETWEEN c.RowEffectiveDate and c.RowExpirationDate
			 AND Active = 1 		 			        
