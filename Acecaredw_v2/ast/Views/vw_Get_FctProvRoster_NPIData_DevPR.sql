...
AS 
    ...
    FROM TMPSALESFORCE_CONTACT AS SF_Contact
	   ...
	   LEFT JOIN (SELECT src.Provider_Name__c, src.County__c
				FROM(SELECT loc.Provider_Name__c, loc.County__c , ROW_NUMBER() OVER (PARTITION BY loc.Provider_Name__c ORDER BY loc.CreatedDate DESC) ARN
					   FROM dbo.tmpSalesforce_Location__c  Loc WHERE isnull(loc.Chapter,'') <> ''
				    )src
