...
INSERT INTO ' + @TgtTblName + ' (CodeVer,adiKey,ClientMemberKey,ClientKey,AttribNPI, EffDate, TermDate, CurEffDate, LoadDate,InsertRunID,PlanMatchFlg,NPIMatchFlg, RecStatus, MaxDateFlg)
SELECT ' + char(39) + @CodeVer + char(39) + ',p.adiKey, p.ClientMemberKey, p.ClientKey, p.AttribNPI, p.EffDate, p.TermDate, p.CurEffDate, convert(date,p.LoadDate), ' + char(39) +  @InsertRunID + char(39) + ',PlanMatchFlg,NPIMatchFlg,' + char(39) +  'N' + char(39) + ',0 
FROM #tmpTable p WHERE p.PlanMatchFlg = 1 AND NPIMatchFlg = 1
...
FROM #tmpTable p WHERE p.PlanMatchFlg = 0 OR NPIMatchFlg = 0
'
...
END
/***
EXEC adw.z_CreateUHCMembers '2020-01-01','2021-04-30','2021-05-03'

SELECT * FROM dbo.z_tmp_AttribMembers			WHERE	ClientKey IN (1) AND Convert(date,CreateDate)	= '2021-05-03'
SELECT * FROM dbo.z_tmp_AttribMembers_Failed WHERE	ClientKey IN (1)	AND EffectiveAsOfDate			= '2021-05-03'
...
DECLARE @EffectiveAsOfDate DATE = '2021-04-15'
