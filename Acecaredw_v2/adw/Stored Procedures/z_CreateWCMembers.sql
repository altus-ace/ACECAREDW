...
/***
EXEC adw.z_CreateWCMembers '2021-01-01','2021-04-30'

TRUNCATE TABLE dbo.z_tmp_AttribMembers WHERE ClientKey = 2
SELECT *  FROM dbo.z_tmp_AttribMembers  WHERE ClientKey = 2
SELECT * FROM dbo.z_tmp_AttribMembers WHERE ClientKey = 2 AND  ActiveFlg = 1 AND LoadDate = (SELECT MAX(LoadDate) FROM dbo.z_tmp_AttribMembers WHERE ClientKey = 2 AND  ActiveFlg = 1)
SELECT * FROM dbo.z_tmp_AttribMembers_Failed WHERE	ClientKey IN (2)
...
LEFT JOIN adw.MbrWlcProviderLookup plu ON p.Prov_ID  = plu.Prov_ID
***/
